# Pipeline and reproduction guide

How the NAKO rib-cage shape model and the manuscript's analyses are produced: repository
layout, environment, data configuration, run presets, every pipeline stage with its outputs,
and methodology notes. The released model itself is described in
